...
def signal_handler(signum, frame):
    ...
    with interrupt_lock:
        ...
        if not interrupted:
            ...
            print("Press Ctrl+C again within 1 second to force exit.")
            interrupted = True
...
def get_existing_files_info(directory: Path, recursive: bool = False) -> Dict[Path, str]:
...
def should_process_file(input_path: Path, output_path: Path, existing_files: Dict[Path, str]) -> bool:
...
def process_file_with_timeout(input_path: Path, output_path: Path, timeout: int = 300) -> Tuple[Path, bool, str]:
    """
...
def process_directory(input_dir: Path, output_dir: Path, recursive: bool = False,
                     max_workers: int = None, timeout: int = 300) -> tuple[int, int, int]:
    """
    ...
    with tqdm(total=total_files, desc="Processing presentations",
             unit="file", ncols=150) as pbar:

        # Process files in parallel
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            # Submit all tasks
            future_to_path = {
                executor.submit(process_file_with_timeout, input_path, output_path): input_path
            ...
            for future in as_completed(future_to_path):
                ...
                try:
                    ...
                    if success:
                        ...
                        failure_count += 1
                        print(f"\nError processing {input_path}: {error}", file=sys.stderr)
                except TimeoutError:
                    failure_count += 1
                    input_path = future_to_path[future]
                    print(f"\nTimeout processing {input_path}: Operation took longer than {timeout} seconds", file=sys.stderr)
                    future.cancel()
                except Exception as e:
                    failure_count += 1
                    input_path = future_to_path[future]
                    print(f"\nUnexpected error processing {input_path}: {str(e)}", file=sys.stderr)

                pbar.update(1)
                pbar.set_description(f"Processed {input_path.name}")

    return success_count, failure_count, skipped_count

def main():
    parser = argparse.ArgumentParser(description="Invert colors in PowerPoint presentations")
    parser.add_argument("input", type=Path, help="Input file or directory")
    parser.add_argument("-o", "--output", type=Path, help="Output file or directory (optional)")
    parser.add_argument("-w", "--workers", type=int, help="Number of worker threads (default: CPU count * 4)")
    parser.add_argument("-r", "--recursive", action="store_true", help="Process subdirectories recursively")
    # TODO: This argument does not work correctly
    parser.add_argument("-t", "--timeout", type=int, default=300, help="Timeout in seconds for processing each file (default: 300)")
    parser.add_argument("-f", "--force", action="store_true", help="Force processing of all files, even if they exist in output")
    ...
    if args.input.is_file():
        ...
        if not args.force and args.output.exists():
            ...
            if not should_process_file(args.input, args.output, existing_files):
                print(f"File already processed: {args.output}")
                sys.exit(0)

        print(f"Processing: {args.input}")
        _, success, error = process_file_with_timeout(args.input, args.output, args.timeout)
        if success:
    ...
    # Process directory
        ...
        if not interrupted:
            print(f"\nProcessing complete:")
            print(f"Successfully processed: {success_count}/{total} files")
            if failure_count > 0:
                print(f"Failed to process: {failure_count}/{total} files")
            if skipped_count > 0:
                print(f"Skipped (already processed): {skipped_count}/{total} files")
